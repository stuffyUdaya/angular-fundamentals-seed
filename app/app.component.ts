...
@Component({
    ...
    <ul>
    <li *ngFor='let passenger of passengers; let i = index;'><span class='status' [ngStyle]="{backgroundColor: (passenger.checkedIn ? 'black' : 'blue')}" ></span> {{i}} : {{passenger.fullName}}</li>
    </ul>
    </div>
...
export class AppComponent{
    ...
    passengers : Passenger[] = [
        { 'id':1, 'fullName': 'Ross', 'checkedIn': true },
        {'id':2, 'fullName': 'Dan', 'checkedIn': false},
        {'id':3, 'fullName': 'Thomas', 'checkedIn': true}
    ];
    constructor(){
